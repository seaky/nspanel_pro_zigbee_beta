        ...
        if (
          ...
          let checkOrder = (members, left, right, iteration) => {
            ...
            let switchIndex = members.findIndex(
              (_, i) =>
                i &&
                isMemberOptional.isMemberOptional(members[i - 1].node) !==
                  isMemberOptional.isMemberOptional(members[i].node),
            )
            if (iteration < switchIndex && iteration + 1 !== switchIndex) {
              return checkGroupSort(left, right)
            }
            if (
              isMemberOptional.isMemberOptional(left.node) !==
              isMemberOptional.isMemberOptional(right.node)
            ) {
              return (
                isMemberOptional.isMemberOptional(left.node) !==
