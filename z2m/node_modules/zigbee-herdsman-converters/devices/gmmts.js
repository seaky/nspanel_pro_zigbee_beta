...
function ticmeterConverter(msg) {
    ...
    keys.forEach((key) => {
        ...
        if (found) {
            ...
            switch (found.type) {
                case STRING:
                    if (buffer_1.Buffer.isBuffer(msg.data[key])) {
...
const definitions = [
    {
        ...
        fromZigbee: [fromZigbee_1.default.meter_identification, fzLocal.ticmeter_cluster_fz, fzLocal.ticmeter_ha_electrical_measurement, fzLocal.ticmeter_metering],
        toZigbee: tzLocal,
                ...
                if (endpoint.clusters[CLUSTER_TIC].attributes !== undefined && endpoint.clusters[CLUSTER_TIC].attributes != undefined) {
                    ...
                    if (globalStore.getValue(device, 'contract_type') == undefined) {
                        if (attr.contractType !== undefined && attr.contractType != null) {
                            ...
                            if (buffer_1.Buffer.isBuffer(string)) {
        ...
        onEvent: async (type, data, device, options) => {
            ...
            if (data.data) {
                ...
                if (data.data.contractType !== undefined) {
                    let contractType;
                    if (buffer_1.Buffer.isBuffer(data.data.contractType)) {
            ...
            if (type === 'stop') {
                ...
                const seconds = options && options.refresh_rate ? Number(options.refresh_rate) : DEFAULT_POLL_INTERVAL;
