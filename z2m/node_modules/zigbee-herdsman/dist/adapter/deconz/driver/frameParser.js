    ...
    try {
        ...
        if (response.status !== 0) {
            if (response.status !== 5) {
                logger_1.logger.debug('DATA_CONFIRM RESPONSE - seqNr.: ' + response.seqNr + ' status: ' + response.status, NS);
            }
            return null;
        }
        ...
        logger_1.logger.debug('DATA_CONFIRM RESPONSE - destAddr: 0x' + destAddr + ' request id: ' + response.requestId + ' confirm status: ' + response.confirmStatus, NS);
...
function parseReadReceivedDataResponse(view) {
    ...
    try {
        ...
        if (response.status != 0) {
            if (response.status !== 5) {
                logger_1.logger.debug('DATA_INDICATION RESPONSE - seqNr.: ' + response.seqNr + ' status: ' + response.status, NS);
            }
            return null;
        }
        ...
        logger_1.logger.debug('DATA_INDICATION RESPONSE - seqNr. ' +
            response.seqNr +
            ' srcAddr: 0x' +
            srcAddr +
            ' destAddr: 0x' +
            ...
            ' lqi: ' +
            response.lqi, NS);
        logger_1.logger.debug('response payload: ' + payload, NS);
...
function parseGreenPowerDataIndication(view) {
    try {
        ...
        if (!(lastReceivedGpInd.srcId === ind.srcId &&
            ...
            logger_1.logger.debug(`GP_DATA_INDICATION - src id: 0x${ind.srcId.toString(16)} cmd id: 0x${ind.commandId.toString(16)} frameCounter: 0x${ind.frameCounter.toString(16)}`, NS);
