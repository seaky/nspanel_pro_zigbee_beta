...
class Group extends entity_1.default {
    ...
    async write(clusterKey, attributes, options) {
        ...
        try {
            const frame = Zcl.Frame.create(Zcl.FrameType.GLOBAL, options.direction, true, options.manufacturerCode, options.transactionSequenceNumber ?? zclTransactionSequenceNumber_1.default.next(), 'write', cluster.ID, payload, {}, options.reservedBits);
    ...
    async read(clusterKey, attributes, options) {
        ...
        const frame = Zcl.Frame.create(Zcl.FrameType.GLOBAL, options.direction, true, options.manufacturerCode, options.transactionSequenceNumber ?? zclTransactionSequenceNumber_1.default.next(), 'read', cluster.ID, payload, {}, options.reservedBits);
    ...
    async command(clusterKey, commandKey, payload, options) {
        ...
        logger_1.logger.debug(log, NS);
        try {
            const frame = Zcl.Frame.create(Zcl.FrameType.SPECIFIC, options.direction, true, options.manufacturerCode, options.transactionSequenceNumber || zclTransactionSequenceNumber_1.default.next(), command.ID, cluster.ID, payload, {}, options.reservedBits);
            await entity_1.default.adapter.sendZclFrameToGroup(this.groupID, frame, options.srcEndpoint);
        }
        catch (error) {
            error.message = `${log} failed (${error.message})`;
